...
def export_wave(event, wavelet):
  ...
  try:
    ...
    MESSAGE = '\\nNow add this robot to your new wave:\\n'
    MESSAGE_TWO = result+'.'+(os.environ['SERVER_NAME'])[:-12]+'@appspot.com'#FIXME doubleslash
    should_i_reply = True
    ...
    if should_i_reply:
      ...
      if True:#wavelet.creator == 'duncan.hawthorne@googlewave.com':
        ...
        new_blip.append(element.Button(dhelper.domain+'.appspot.com/newwave/', 'Test the robot'))
...
if __name__ == '__main__': 
  ...
  myRobot.register_handler(events.BlipSubmitted, export_wave, context = [events.Context.ALL])
  myRobot.register_handler(events.FormButtonClicked, make_new_wave) 
  CONSUMER_KEY = ''
  CONSUMER_SECRET = ''
